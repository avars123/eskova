...
def get_top_users():
    ...
    for post in data_posts["posts"].values():
        user_id = post["user_id"]
        if user_id not in user_stats:
            user_stats[user_id] = {"posts": 0, "approved": 0, "username": post["username"]}
...
def show_post(message):
    ...
    status_info = ""
    if post["status"] != "pending" and "moderated_by" in post:
    ...
    if post["media_type"] == "text":
        bot.send_message(
            message.chat.id, 
            f"📝 *Текст поста:*\n\n{post['text']}\n\n*Статус:* {post['status']}{status_info}",
            parse_mode="Markdown"
        )
...
def moderation_handler(call):
    ...
    if action == 'approve':
        ...
        try:
            caption = post["text"]
            if caption:
                caption += f"\n\n👁‍🗨 Пост предложен: {post['username']}"
            
            if post["media_type"] == "text":
                bot.send_message(CHANNEL_ID, caption or post["text"])
            elif post["media_type"] == "photo":
                bot.send_photo(CHANNEL_ID, post["file_id"], caption=caption)
            elif post["media_type"] == "video":
                bot.send_video(CHANNEL_ID, post["file_id"], caption=caption)
            elif post["media_type"] == "sticker":
                bot.send_sticker(CHANNEL_ID, post["file_id"])
            elif post["media_type"] == "voice":
                bot.send_voice(CHANNEL_ID, post["file_id"], caption=caption)
            elif post["media_type"] == "video_note":
        ...
        # Здесь можно добавить обработку комментария
...
# Обработчики медиа (остаются аналогично, но с улучшениями)
...
def process_media(message, media_type):
    ...
    if media_type == 'media_group':
        data["posts"][str(post_id)]["media_group_id"] = message.media_group_id
