...
def safe_send_message(chat_id, text, parse_mode="Markdown", **kwargs):
    try:
        ...
        # Если есть ошибка форматирования, отправляем без Markdown
        try:
            return bot.send_message(chat_id, text, parse_mode=None, **kwargs)
        except Exception as e2:
            raise e2

# Функция для безопасного редактирования сообщений с Markdown
def safe_edit_message_text(chat_id, message_id, text, parse_mode="Markdown", **kwargs):
    try:
        if parse_mode == "Markdown":
            text = escape_markdown(text)
        return bot.edit_message_text(text, chat_id, message_id, parse_mode=parse_mode, **kwargs)
    except Exception as e:
        # Если есть ошибка форматирования, редактируем без Markdown
        try:
            return bot.edit_message_text(text, chat_id, message_id, parse_mode=None, **kwargs)
        except Exception as e2:
            raise e2

# Функция для безопасного редактирования подписи
def safe_edit_message_caption(chat_id, message_id, caption, parse_mode="Markdown", **kwargs):
    try:
        if parse_mode == "Markdown":
            caption = escape_markdown(caption)
        return bot.edit_message_caption(caption, chat_id, message_id, parse_mode=parse_mode, **kwargs)
    except Exception as e:
        try:
            return bot.edit_message_caption(caption, chat_id, message_id, parse_mode=None, **kwargs)
        except Exception as e2:
            raise e2
...
def handle_admin_application(message):
    try:
        ...
        if current_state == "admin_app_1":
            ...
            safe_send_message(message.chat.id, "*2. Твой возраст (просто цифра, нам не верить, а для статистики):*")
            ...
            # Формируем сообщение для админов с экранированием
            admin_message = (
                f"{funny_reaction}\n\n"
                "👑 *НОВАЯ ЗАЯВКА НА АДМИНА* 👑\n\n"
                f"🆔 *ID заявки:* #{application_id}\n"
                f"👤 *Пользователь:* {escape_markdown(app_data.get('username', 'Unknown'))}\n"
                f"🔗 *Telegram:* {escape_markdown(app_data.get('telegram_link', 'Не указано'))}\n"
                f"🆔 *User ID:* {user_id}\n"
                f"📅 *Дата подачи:* {datetime.datetime.now().strftime('%d.%m.%Y %H:%M')}\n\n"
                f"*1. Имя:* {escape_markdown(app_data.get('name', 'Не указано'))}\n"
                f"*2. Возраст:* {escape_markdown(app_data.get('age', 'Не указано'))}\n"
                f"*3. Причина:* {escape_markdown(app_data.get('reason', 'Не указано'))}\n"
                f"*4. Опыт:* {escape_markdown(app_data.get('experience', 'Не указано'))}\n"
                f"*5. Решение конфликтов:* {escape_markdown(app_data.get('conflict_solution', 'Не указано'))}\n"
                f"*6. Время:* {escape_markdown(app_data.get('time', 'Не указано'))}\n"
                f"*7. Идея:* {escape_markdown(app_data.get('idea', 'Не указано'))}\n"
                f"*8. Оружие:* {escape_markdown(app_data.get('weapon', 'Не указано'))}\n"
            )
        ...
        if action == "approve":
            ...
            try:
                safe_send_message(
                    ...
                    f"*Кто принял:* {escape_markdown(admin_info)}\n"
            ...
            new_text = f"{call.message.text}\n\n{admin_reaction} {escape_markdown(admin_info)}"
            try:
                safe_edit_message_text(
                    ADMIN_GROUP_ID,
                    call.message.message_id,
            ...
            try:
                safe_send_message(
                    ...
                    f"*Кто отклонил:* {escape_markdown(admin_info)}\n"
            ...
            interview_msg = safe_send_message(
                ADMIN_GROUP_ID,
                f"💬 *Начат процесс собеседования с {escape_markdown(application.get('username', 'Unknown'))}*\n\n"
                f"*Кто проводит:* {escape_markdown(admin_info)}\n"
            ...
            # Обновляем сообщение в группе
            new_text = f"{call.message.text}\n\n🚀 СРОЧНО ПРИНЯТ! {escape_markdown(admin_info)}"
            try:
                safe_edit_message_text(
                    ADMIN_GROUP_ID,
                    call.message.message_id,
        ...
        try:
            ...
            if hasattr(forwarded_msg, 'text') and forwarded_msg.text:
                safe_edit_message_text(
                    ...
                    f"💬 *Собеседование с {escape_markdown(application.get('username', 'Unknown'))}:*\n\n{escape_markdown(forwarded_msg.text)}",
                ...
                safe_send_message(
                    ADMIN_GROUP_ID,
                    f"💬 *Медиа-сообщение от {escape_markdown(application.get('username', 'Unknown'))} во время собеседования*",
                    reply_markup=interview_finish_buttons(app_id)
                )
        except Exception as e:
...
def show_post(message):
    try:
        ...
        if post.get("status") != "pending" and "moderated_by" in post:
            status_info = f"\n\n*Модератор:* {escape_markdown(post['moderated_by'])}"
        
        # Отправка медиа в зависимости от типа
        media_type = post.get("media_type", "text")
        post_text = escape_markdown(post.get('text', ''))
        post_status = escape_markdown(post.get('status', 'unknown'))
        
        if media_type == "text":
            safe_send_message(
                message.chat.id, 
                f"📝 *Текст поста:*\n\n{post_text}\n\n*Статус:* {post_status}{status_info}"
            )
            ...
            try:
                ...
                bot.send_photo(
                    message.chat.id, 
                    post.get("file_id"), 
                    caption=f"{post.get('text', '')}\n\nСтатус: {post.get('status', 'unknown')}{status_info.replace('*', '')}"
                )
    ...
    try:
            ...
            try:
                ...
                try:
                    safe_send_message(
                        ...
                        f"*Кто опубликовал:* {escape_markdown(admin_info)}\n"
        ...
        if action in ['approve', 'reject']:
            try:
                ...
                status_text = {
                    'approve': f"✅ Опубликовано {escape_markdown(admin_info)}",
                    'reject': f"❌ Отклонено {escape_markdown(admin_info)}"
                }.get(action, "")
...
def handle_text(message):
    ...
    try:
        ...
        admin_message = safe_send_message(
            ADMIN_GROUP_ID,
            f"👤 {escape_markdown(username)} (ID {user_id}) предложил пост #{post_id}\n\n{escape_markdown(message.text)}",
            reply_markup=moderation_buttons(post_id)
        )
...
def process_media(message, media_type):
    ...
    try:
        ...
        admin_text = f"👤 {escape_markdown(username)} (ID {user_id}) предложил пост #{post_id} ({media_names[media_type]}){escape_markdown(caption_text)}"
        ...
        try:
            if media_type == 'photo':
                ...
                admin_message = safe_send_message(ADMIN_GROUP_ID, admin_text, reply_markup=moderation_buttons(post_id))
            elif media_type == 'voice':
